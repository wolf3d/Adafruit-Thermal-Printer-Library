...
void processImage(File image) {
  ...
  PImage      img;
  PrintWriter output;
  int         pixelNum, pxNum, byteNum, bytesOnLine = 99, 
    x, y, b, rowBytes, pixelRows, pixelColumns, totalBytes, pixelsLength, lastBit, sum;
  ...
  pixelsLength = img.pixels.length;
  pixelColumns = 16;

  println("totalBytes: " + totalBytes);
  colorData = new int[totalBytes];
  ...
  output.println();
  output.print("const uint8_t " + basename + " [] PROGMEM = {");

  //// Generate body of array
  for (pxNum=pixelNum=byteNum=y=0; y<img.height; y++) { // Each row...
  ...
  pixelRows = totalBytes / pixelColumns;

  println("pixelColumns: " + pixelColumns);
  println("pixelRows: " +pixelRows);

  int k = 0;
  int m = 0;
  output.print("\n");
  for (y=0; y < pixelColumns; y++ ) { // columns

    for (x=0; x < pixelRows; x++ ) { // rows

      m = pixelColumns * x + y;
      // 
      println("m: " + m);
      output.format("0x%02X", colorData[m]); // Write accumulated bits
      if (m + 1< totalBytes) output.print(", ");
      //m +=1;
      k +=1;
      if ( pixelColumns != 0 && k % pixelColumns ==0) output.print("\n");
      if (k == pixelColumns) k = 0;
  ...
  output.flush();
  output.close();
  println("colorData: "  + colorData.length);
  println("Done!");
  exit();
}